...
def close_gamut_boundary_descriptor(GBD_m):
    GBD_m = spherical_to_Jab(GBD_m)

    # Index of first row with values.
    r_t_i = np.argmin(np.all(np.any(np.isnan(GBD_m), axis=-1), axis=1))

    # First row with values.
    GBD_m_t = GBD_m[r_t_i, ...]
    GBD_m_t = GBD_m_t[~np.any(np.isnan(GBD_m_t), axis=-1)]
    if not np.allclose(GBD_m_t, GBD_m_t[0]) or GBD_m_t.shape[0] == 1:
        Jab_t = np.nanmean(
            GBD_m_t[GBD_m_t[..., 0] == np.max(GBD_m_t[..., 0])], axis=0)
        ...
        GBD_m[r_t_i] = np.tile(Jab_t, [1, GBD_m.shape[1], 1])

    # Index of last row with values.
    r_b_i = np.argmin(
        np.flip(np.all(np.any(np.isnan(GBD_m), axis=-1), axis=1)))
    r_b_i = GBD_m.shape[0] - 1 - r_b_i

    # Last row with values.
    GBD_m_b = GBD_m[r_b_i, ...]
    GBD_m_b = GBD_m_b[~np.any(np.isnan(GBD_m_b), axis=-1)]
    if not np.allclose(GBD_m_b, GBD_m_b[0]) or GBD_m_b.shape[0] == 1:
        Jab_b = np.nanmean(
            GBD_m_b[GBD_m_b[..., 0] == np.min(GBD_m_b[..., 0])], axis=0)

        if r_b_i != GBD_m.shape[0] - 1:
            r_b_i += 1

        warning('Closing bottom of GBD at row {0} with value {1}'.format(
            r_b_i, Jab_b))

        GBD_m[r_b_i] = np.tile(Jab_b, [1, GBD_m.shape[1], 1])

    GBD_m = Jab_to_spherical(GBD_m)

    return GBD_m


def fill_gamut_boundary_descriptor(GBD_m):
    GBD_m = spherical_to_Jab(GBD_m)

    GBD_m_i = np.copy(GBD_m)

    shape_r, shape_c = GBD_m.shape[0], GBD_m.shape[1]
    ...
    # vice-versa.
    if np.any(np.isnan(GBD_m_i[..., 0])) or np.any(np.isnan(GBD_m_i[..., -1])):
        warning(
    ...
    if np.any(np.isnan(GBD_m_i[0, ...])) or np.any(np.isnan(GBD_m_i[-1, ...])):
        ...
        GBD_m_f = orient(GBD_m_i, 'Flop')
        GBD_m_i = np.vstack([GBD_m_f, GBD_m_i, GBD_m_f])

    shape_r, shape_c = GBD_m_i.shape[0], GBD_m_i.shape[1]
    mask = np.any(~np.isnan(GBD_m_i), axis=-1)
    for i in range(GBD_m.shape[-1]):
        x = np.linspace(0, 1, shape_r)
        y = np.linspace(0, 1, shape_c)
        x_g, y_g = np.meshgrid(x, y, indexing='ij')
        values = GBD_m_i[mask]

        GBD_m_i[..., i] = scipy.interpolate.griddata(
            (x_g[mask], y_g[mask]),
            values[..., i], (x_g, y_g),
            method='linear')

    return Jab_to_spherical(GBD_m_i[r_slice, c_slice, :])


def sample_gamut_boundary_descriptor(GBD_m, theta_alpha):
    GBD_m = spherical_to_Jab(GBD_m)
    theta, alpha = tsplit(theta_alpha)

    GBD_m_s = np.zeros(list(theta_alpha.shape[:2]) + [3])

    x = np.linspace(0, 1, GBD_m.shape[0])
    y = np.linspace(0, 1, GBD_m.shape[1])
    x_g, y_g = np.meshgrid(x, y, indexing='ij')
    ...
    for i in range(GBD_m.shape[-1]):
        GBD_m_s[..., i] = scipy.interpolate.griddata(
            (np.ravel(x_g), np.ravel(y_g)),
            np.ravel(GBD_m[..., i]), (theta_i, alpha_i),
            method='linear')

    GBD_m_s = Jab_to_spherical(GBD_m_s)

    return GBD_m_s


def tessellate_gamut_boundary_descriptor(GBD_m):
    if is_trimesh_installed():
        import trimesh

        vertices = spherical_to_Jab(GBD_m)
...
if __name__ == '__main__':
    ...
    GBD_t_r = GBD_t_s[0]
    for i, GBD_t in enumerate(GBD_t_s[1:]):
