...
def gamut_boundary_descriptor_Morovic2000(
    ...
    for i in np.arange(m):
        for j in np.arange(n):
            ...
            GBD_m[i, j] = r_theta_alpha[i_j[np.argmax(r[i_j])]]
    ...
    #         GBD_m[p_i, a_i] = r_theta_alpha_i
    #         r_m[p_i, a_i] = r[i]

    if close_callable is not None:
        GBD_m = close_gamut_boundary_descriptor(
            GBD_m,
            **filter_kwargs(close_gamut_boundary_descriptor,
                            **close_callable_args))

    if fill_callable is not None:
        GBD_m = fill_gamut_boundary_descriptor(
            GBD_m,
